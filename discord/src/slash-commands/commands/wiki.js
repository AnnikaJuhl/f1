...
const driverNames = require('../data/driverNames.json');
	...
	data: new SlashCommandBuilder()
		...
		.addStringOption(option =>
			option.setName('driver-name')
				.setDescription('Information on any driver')
				.setAutocomplete(true)
      ),

	async autocomplete(interaction) {
    const focusedValue = interaction.options.getFocused();
     const filtered = driverNames.filter(name => 
...
async execute(interaction) {
      const selected = interaction.options.getString('driver-name');
